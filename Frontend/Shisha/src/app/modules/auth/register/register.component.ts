...
export class RegisterComponent implements OnInit {
  ...
  public register(): void {
    ...
    setTimeout(() => {
      ...
      var response = this.authService.register(body).subscribe({
        ...
        error: error => {
          if(error.error == "There already is an user registered with that email!"){
  ...
  public getPasswordErrors() : String[] {
    const controlErrors: ValidationErrors | null | undefined = this.registerForm.get('password')?.errors;
    var result : String[] = [];
    if (controlErrors != null && controlErrors != undefined) {
      Object.keys(controlErrors).forEach(keyError => {
        if(controlErrors[keyError] == false && keyError == 'upper'){
          ...
          result.push("Your password must be at least " +
          controlErrors[keyError].requiredLength + " characters long.");
        }
      });
    }
    return result;
  ...
  public checkName() : boolean{
    ...
    (this.registerForm.controls['firstName'].dirty || this.registerForm.controls['firstName'].touched)){
      return true;
    }
    if(this.registerForm.controls['lastName'].invalid &&
    (this.registerForm.controls['lastName'].dirty || this.registerForm.controls['lastName'].touched)){
      return true;
    }
    return false;
  }
